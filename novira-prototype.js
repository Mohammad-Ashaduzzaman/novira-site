(function () {
  ...
  function currentSection() {
    ...
    if ([pages.solutions, pages.logistics, pages.coldChain, pages.insurance, pages.manufacturing, pages.assetProtection].indexOf(file) !== -1) return 'solutions';
    if ([pages.platform, pages.track, pages.novacheck, pages.novaproof, pages.novalink, pages.novaprotect, pages.dost].indexOf(file) !== -1) return 'platform';
    if ([pages.products, pages.compare, pages.pricing, pages.build, pages.checkout, pages.quote].indexOf(file) !== -1) return 'products';
    if ([pages.blog, pages.cargoBlog, pages.chainBlog, pages.parametricBlog, pages.nobodyBlog, pages.tempBlog, pages.trackingBlog, pages.claimsBlog].indexOf(file) !== -1) return 'resources';
    if ([pages.resources, pages.docs, pages.gettingStarted, pages.productsOverview, pages.security, pages.faq, pages.api, pages.portal, pages.useCases, pages.coldUse, pages.insuranceUse, pages.manufacturingUse, pages.theftUse, pages.journey].indexOf(file) !== -1) return 'resources';
    if (file === pages.about) return 'about';
    if (file === pages.contact) return 'contact';
    if (file === pages.login) return 'company';
    return 'home';
  }
  ...
  function megaPanel(kind) {
    var panels = {
      solutions: {
        ...
        all: pages.solutions,
        allLabel: 'Open solutions overview',
        cols: [
          ['Solution network', [
      ...
      resources: {
        ...
        copy: 'Articles, documentation, and use-case playbooks grouped into one resource network.',
        all: pages.resources,
        ...
        cols: [
          ...
          ['Documentation', [
            [pages.docs, 'Documentation Home', 'Guides, FAQs, API notes, and trust model'],
            [pages.gettingStarted, 'Getting Started', 'First deployment and account setup'],
            [pages.productsOverview, 'Product Docs', 'Device and platform selection guide'],
          ...
          ['Use Cases', [
            [pages.useCases, 'Use Case Library', 'Cold, claims, compliance, theft, and risk'],
            [pages.coldUse, 'Cold Chain', 'Verified breach and response workflow'],
  ...
  function injectUnifiedNav() {
    ...
    header.innerHTML =
      '<div class="site-nav-inner">' +
        ...
        '<nav class="site-main-nav" aria-label="Primary navigation">' +
          ...
          '<a class="site-nav-link" data-section="about" href="' + routeHref(pages.about) + '">About</a>' +
          '<a class="site-nav-link" data-section="contact" href="' + routeHref(pages.contact) + '">Contact</a>' +
        '</nav>' +
    ...
    header.querySelectorAll('[data-section="' + section + '"]').forEach(function (node) {
      node.classList.add('is-active');
      if (node.classList.contains('site-nav-item')) {
        var link = node.querySelector('.site-nav-link');
